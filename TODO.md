...
**Per-slot caps**（2026-05-05 三次调整）：原本所有 file_type 共用同一 1 GB cap，对基因列表 / motif 数据库（注定只是 KB–MB 级别的 cluster→ID tab 表 / .meme 文件）来说太松——粗心或恶意上传几百 MB 的 .txt 是合法的。改成 per-`file_type` lookup：

- [files.py](apps/pmet_backend/api/routes/files.py) `_RAW_MAX_BYTES_BY_TYPE` dict：`genes / meme = 2 MB`、`fasta / gff3 = 1 GB`。`_enforce_raw_upload_cap` 和 `_store_upload` 都按 `file_type` 取对应 cap。decompressed cap 仍 5 GB，但 gzip 路径只对 `fasta / gff3` 开放（`GZIP_ENABLED_FILE_TYPES`），所以 genes/meme 永远走 raw。
- [FileUpload.tsx](apps/pmet_frontend/components/FileUpload.tsx) 加 `sizeLimitText` prop，渲染在"支持格式"行下方——caller 决定文案。
- [submit/page.tsx](apps/pmet_frontend/app/submit/page.tsx) 每个 slot 传具体 limit 文案；顶部 hint 改成精确说明（"FASTA/GFF3 上限 1 GB / 5 GB；基因列表与 motif 数据库 2 MB"）。
- 单测 [test_upload_routes.py](apps/pmet_backend/test_upload_routes.py) 加 `test_upload_rejects_oversize_genes_small_cap` / `_meme_small_cap`，用 `patch.dict(_RAW_MAX_BYTES_BY_TYPE, {...})` 验证 per-type lookup 真的生效。

curl 验证：3 MB genes/meme → 413 "2 MB size cap"；5 MB FASTA → 200（共用 1 GB cap）；1 KB genes → 200。MCP browser zh/en 双语下，4 个 slot 各自显示对应 limit（"≤ 1 GB 原始 / gzip 解压 ≤ 5 GB" 或 "≤ 2 MB"）。25/25 单测 pass。
