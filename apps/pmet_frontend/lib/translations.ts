...
const en = {
  ...
  'submit.upload.size_hint': 'Sequence (FASTA) and annotation (GFF3) accept up to 1 GB raw / 5 GB after gzip decompression — larger genomes (wheat, human) should use a pre-computed species or the offline CLI. Gene lists and motif DBs are capped at 2 MB.',
...
const zh: Record<keyof typeof en, string> = {
  ...
  'submit.upload.size_hint': '基因组 / 区段序列（FASTA）和注释（GFF3）上限：原始 1 GB / gzip 解压后 5 GB；更大的基因组（小麦、人类等）请用预计算物种或线下 CLI。基因列表与 motif 数据库上限 2 MB。',
