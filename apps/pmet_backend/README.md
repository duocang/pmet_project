...
| `PUBLIC_BASE_URL`         | unset (read from `deploy/configure/public_base_url.txt`) | bare-domain deployment URL (e.g. `https://pmet.example.org`); used to build absolute links in outbound emails (task-detail page + partial-result API). Unset → emails ship without a clickable button |
...
| `PUBLIC_BASE_URL`           | 未设（读 `deploy/configure/public_base_url.txt`） | 部署的裸域名 URL（例如 `https://pmet.example.org`）；用来在外发邮件里拼绝对链接（任务详情页 + partial-result API）。未设 → 邮件照发，但没有可点的按钮 |
