...
DEMO_FILES: dict[tuple[str, str], tuple[str, str]] = {
    ("promoters_pre", "genes"): ("demo_promoters/example_genes.txt", "example_genes.txt"),
    ("promoters", "genes"):     ("demo_promoters/example_genes.txt", "example_genes.txt"),
    ("promoters", "fasta"):     ("TAIR10.fasta",                     "TAIR10.fasta"),
    ("promoters", "gff3"):      ("TAIR10.gff3",                      "TAIR10.gff3"),
    ("promoters", "meme"):      ("demo_promoters/example_motif.meme", "example_motif.meme"),
    ("intervals", "genes"):     ("demo_intervals/peaks.txt",         "peaks.txt"),
    ("intervals", "fasta"):     ("demo_intervals/intervals.fa",      "intervals.fa"),
    ("intervals", "meme"):      ("demo_intervals/motif.meme",        "motif.meme"),
}


@router.get("/example-result")
async def get_example_result():
    full = config.PROJECT_ROOT / "data" / "demo_pmet_analysis" / "example_pmet_result.txt"
    if not full.exists():
