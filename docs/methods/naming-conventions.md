...
| `nginx_link.txt` | base URL written into outbound emails (e.g. `https://pmet.example.org`) | partial-result link rendering |
...
| `nginx_link.txt` | 写进外发邮件的 base URL（如 `https://pmet.example.org`） | partial-result 链接渲染 |
