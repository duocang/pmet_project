...
| `run_homotypic.py` | End-to-end homotypic stage; combines every helper above + `build/indexing_fimo_fused`. The promoter / promoter_gap workflows now invoke this rather than wiring the steps inline. |
...
| `run_homotypic.py` | 端到端 homotypic stage；组合上面所有 helper + `build/indexing_fimo_fused`。promoter / promoter_gap 工作流现在调它，而不是把步骤一条条写在 shell 里。 |
