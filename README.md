...
| [4. The four workflows](#en-4)        | [11. Migration history](#en-11) | [→ 跳到汉文](#cn)                             |
...
If you're new, [§10 Troubleshooting](#en-10) lists the six things that trip up most first-time users (chromosome-name mismatches between GFF3 and FASTA, FIMO vs `:` in headers, gene-list format, missing R packages, port conflicts, stuck "running" tasks). Skim it before running on your own data.
...
| `data/configure/email_credential.txt` | so users get a download-link email when their task finishes; without it task completion still works, just no notification | 5 lines: `username` / `password` (Gmail app-password recommended) / `from_address` / `smtp_server` / `port`. Gitignored — **never commit it**. |
...
```bash
make up          # build images + start the stack (5–10 min on first build, seconds afterwards)
make ps          # container status — should show 6 services up + healthy
...
- **Open** **http://localhost:5960** in a browser. nginx fronts the frontend at `/` and the API at `/api/...` from the same origin.
- **Submit a task**: pick one of the four entry-point cards on the home page (matches the four workflows in [§4](#en-4)), fill the form, hit submit. You'll get an email when it's done with a link to download the zip.
- **Look up your tasks**: `/tasks?email=you@example.com` (also reachable from the home page nav).
...
### 8.5 Editing app code (bind mounts vs rebuild)
...
| [4. 四种工作流](#cn-4)         | [11. 迁移历史](#cn-11)  | [→ Jump to English](#en)             |
...
新人请先扫一眼 [§10 排错](#cn-10)，那里列了新手最容易撞的 6 堵墙（GFF3 与 FASTA 染色体名不一致、FIMO 不认 header 里的 `:`、gene list 格式、缺 R 包、5960 端口被占、任务卡在 running）。跑自己数据之前先看，能救一小时。
...
| `data/configure/email_credential.txt` | 任务跑完给用户发下载链接邮件；缺它任务还能跑，只是没通知 | 5 行：`username` / `password`（推荐 Gmail app password）/ `from_address` / `smtp_server` / `port`。Gitignored —— **不要提交**。 |
...
```bash
make up          # 构建镜像 + 起栈（首次 build 5–10 分钟，之后秒级）
make ps          # 容器状态 —— 应看到 6 个服务 up + healthy
make logs        # 跟所有服务的日志
make down        # 停栈
...
- **打开** **http://localhost:5960**。nginx 把前端挂在 `/`、API 挂在 `/api/...`，同源出口。
- **提交任务**：首页四个入口卡片（对应 [§4](#cn-4) 的四个 workflow），选一个、填表单、提交。任务跑完会给你的邮箱发一封带 zip 下载链接的邮件。
- **看自己任务**：`/tasks?email=you@example.com`（首页导航也能进）。
