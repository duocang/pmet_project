...
The Python helpers used:
    ...
    parse_memefile.py, calculateICfrommeme_IC_to_csv.py,
...
def main() -> int:
    ...
    parser.add_argument(
        "--gff3sort",
        type=Path,
        default=REPO_ROOT / "scripts" / "gff3sort" / "gff3sort.pl",
    )
    ...
    ic_txt = out / "IC.txt"
    memefiles_ic_dir = out / "memefiles_ic"
    ...
    memefiles_ic_dir.mkdir(parents=True, exist_ok=True)
    run([
        "python3", str(PY_DIR / "parse_memefile.py"),
        str(args.meme), str(memefiles_ic_dir) + "/",
    ])
    run([
        "python3", str(PY_DIR / "calculateICfrommeme_IC_to_csv.py"),
        str(memefiles_ic_dir) + "/", str(ic_txt),
    ])
    shutil.rmtree(memefiles_ic_dir)

    # ---- 7. FIMO + PMETindex via index_fimo_fused ----
    # Upper-case MOTIF header lines so motif ids are consistent with
    # parse_memefile.py (which also upper-cases them) — pair_parallel
    # joins fimohits ↔ IC ↔ binomial_thresholds by motif id.
