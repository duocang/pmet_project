...
DATA_DIR="$PROJECT_ROOT/data/indexing/demo"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  -d, --data DIR         Data directory (default: data/indexing/demo)"
...
case "$VERSION" in
    "c")
        ...
        FALLBACK="$PROJECT_ROOT/src/indexing/from_c/build/index_c"
        ;;
    "cpp")
        EXECUTABLE="$PROJECT_ROOT/build/index_cpp"
        FALLBACK="$PROJECT_ROOT/src/indexing/from_cpp/build/index_cpp"
        ;;
    "fused")
        EXECUTABLE="$PROJECT_ROOT/build/index_fimo_fused"
        FALLBACK="$PROJECT_ROOT/src/indexing/fused_fimo/build/index_fimo_fused"
        ;;
    *)
...
if [ ! -f "$EXECUTABLE" ]; then
    ...
    echo "Please run: bash scripts/build_all.sh indexing-$VERSION"
    exit 1
fi
