...
void arraylst_free(ARRAYLST_T *arraylst) {
  arraylst_destroy(free, arraylst);
}
...
/*
 * arraylst_peek
 * Gets the item at the last index. Returns NULL if no items.
 */
void *arraylst_peek(ARRAYLST_T *arraylst) {
  if (arraylst == NULL)                     die("arraylst_get: arraylst is NULL!\n");
  if (arraylst->size == 0) return NULL;
  return arraylst->array[arraylst->size - 1];
}
...
void *arraylst_remove_range(int index, int count, void (*optional_item_destructor)(void*), ARRAYLST_T *arraylst) {
  ...
  if (optional_item_destructor != NULL) {
    item = NULL;
    arraylst_apply_range(optional_item_destructor, index, count, arraylst);
  } else {
