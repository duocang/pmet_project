...
The `.png` files next to `fingerprints.txt` are full-page screenshots of the web UI captured right before the monorepo merge — home, submit, intervals task, etc. Nothing reads them automatically; they exist as a "this is what the page used to look like" reference for an eyeball diff after a UI change.
...
`fingerprints.txt` 旁边那一堆 `.png` 是迁 monorepo 之前对 web UI （首页、submit、intervals 任务等）整页的截图。没有脚本会自动读它们，存在的目的只是 —— 改完 UI 后可以拿出来肉眼对比"以前长这样"。
