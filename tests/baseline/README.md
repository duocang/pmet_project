...
- **core_demo_run_indexing_{c,cpp,fused}** — runs `PMET_project/scripts/run_indexing.sh -v <ver>` against `data/cli/indexing/demo` and hashes every produced file.
- **core_demo_run_pairing** — runs `PMET_project/scripts/run_pairing.sh` against `data/cli/pairing/demo` and hashes outputs.
