...
t0=time.time()
fimo = pd.read_csv('fimo_ZAT18.txt', sep='\t', index_col=None, header=0)
fimo = fimo.values
print(fimo[0,0])
...
for i in range(1,fimo.shape[0]):
    if (fimo[i,1] != fimo[start_pos,1]):
        ...
        motif_hits = motif_hits[0:min(5,motif_hits.shape[0]),:] #argument here
        binom_p = []
...
#d = time.time() - t0
#
#print("binomial test old method: %.2f s." % d)
