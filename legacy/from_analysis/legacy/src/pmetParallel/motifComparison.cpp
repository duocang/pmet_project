...
void motifComparison::colocTest(long universeSize, double ICthreshold, const std::string &clusterName,
  ...
  std::cout << std::endl << "genesInUniverseWithBothMotifs" << std::endl;
  ...
  std::cout << std::endl << clusterSize << std::endl;
  std::cout << std::endl << universeSize << std::endl;
  std::cout << std::endl << numGenesWithBothMotifs << std::endl;
  ...
  std::cout << std::endl << "genesInClusterWithBothMotifs" << std::endl;
  // 遍历并打印前五个元素（如果存在）
  for (size_t i = 0; i < count; ++i) {
      std::cout << genesInClusterWithBothMotifs[i] << std::endl;
  }
  ...
  std::cout << std::endl << "fadsfadsfadsfasf发腮房东" << std::endl;
  std::cout << std::endl << pval << std::endl;
}
