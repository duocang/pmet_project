#!/usr/bin/env python3
"""Convert GFF3 mRNA lines to a BED-like file, extracting the transcript name from an attribute key (e.g. 'ID:')."""
...
if findstr[-1] != ':':
    sys.stderr.write(
        'WARNING: The field string does not end with a colon sign. '
        'This might be correct, depending on the GFF3 file, but double check your results'
    )

genelines = pd.read_csv(infile, sep='\t', index_col=None, header=None, low_memory=False).values
