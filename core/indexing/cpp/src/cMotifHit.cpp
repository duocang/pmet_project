...
std::ostream& operator<<(std::ostream& ostr, const cMotifHit& hit) {
  ...
  ostr << hit.startPos << "\t" << hit.endPos << "\t" << hit.strand << "\t" << std::scientific << std::setprecision(10)
       << hit.score << "\t" << hit.pVal << "\t" << hit.sequence;

  if (hit.binScore >= 0)
    ostr << "\t" << std::scientific << std::setprecision(10) << hit.binScore;
