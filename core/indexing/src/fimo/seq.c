  ...
  (int start, int stop, SEQ_T* a_sequence)
{
  assert(a_sequence != NULL);
  assert((stop - start) >= 0);
  char *sequence = a_sequence->sequence;
...
)
{
  ...
  for (i_seq = 0; i_seq < sequence->length; i_seq++) {
  ...
  if (alph_size_pairs(alph) == 2) { 
    ...
    sequence->gc = (int *)mm_malloc(sizeof(int) * sequence->length);
    gc = sequence->gc;
   ...
   SEQ_T** sequences)
{
  ...
  for (i_seq = 0; i_seq < num_seqs; i_seq++) {
    this_length = sequences[i_seq]->length;
   ...
   SEQ_T** sequences)
{
  ...
  max_length = 0;
  for (i_seq = 0; i_seq < num_seqs; i_seq++) {
    this_length = strlen(sequences[i_seq]->name);
    if (this_length > max_length) {
      max_length = this_length;
    }
  }
