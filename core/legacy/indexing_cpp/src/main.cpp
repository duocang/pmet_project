...
int main(int argc, const char* argv[]) {
  ...
  std::string fimoHitsPath = outDir + fimoHitsDir;
  mkdir(outDir.c_str(), 0755);
  mkdir(fimoHitsPath.c_str(), 0755);
  ...
  for (long f = 0; f < numFimoFiles; f++) {
    ...
    message << (f + 1) << " of " << numFimoFiles << ", processing FIMO result file" << ": " << fimoFiles[f]
            << std::endl;
    ...
    // motif binscore
    btFile << btVals.first << "\t" << std::scientific << std::setprecision(10) << btVals.second << std::endl;
  }
  btFile.close();
